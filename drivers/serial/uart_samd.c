...
static int uart_samd_fifo_fill(struct device *dev, const u8_t *tx_data, int len)
{
...
static void uart_samd_irq_tx_enable(struct device *dev)
{
	SercomUsart *regs = DEV_CFG(dev)->regs;
...
static int uart_samd_irq_tx_ready(struct device *dev)
{
...
#if CONFIG_UART_INTERRUPT_DRIVEN
	.fifo_fill = uart_samd_fifo_fill,
	.fifo_read = uart_samd_fifo_read,
	.irq_tx_enable = uart_samd_irq_tx_enable,
	.irq_tx_disable = uart_samd_irq_tx_disable,
	.irq_tx_ready = uart_samd_irq_tx_ready,
