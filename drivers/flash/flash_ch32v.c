...
#define DT_DRV_COMPAT wch_ch32v_flash
...
static int flash_ch32v_read(const struct device *dev, off_t offset, void *data, size_t len)
{
	...
	uint32_t flash_addr = config->base_addr + offset;
	...
	memcpy(data, (const void *)(uintptr_t)flash_addr, len);
...
static int flash_ch32v_write(const struct device *dev, off_t offset, const void *data, size_t len)
{
	...
	for (size_t p = 0; p < len; p += write_block_size) {
		uint32_t page_addr = config->base_addr + offset + p;

		/* Set FTPG bit to enable fast programming mode */
		regs->CTLR |= FLASH_CTLR_PAGE_FTPG;

		/* Set BUFRST bit to clear internal buffer */
		...
		/* Write words to the page address consecutively */
		for (int i = 0; i < (write_block_size >> 2); i++) {
			*(volatile uint32_t *)(uintptr_t)(page_addr + i * 4) = src[p / 4 + i];

			/* Set the BUFLOAD bit to load it into cache */
...
static int flash_ch32v_erase(const struct device *dev, off_t offset, size_t size)
{
	...
	for (off_t addr = offset; addr < offset + size; addr += erase_block_size) {
		...
		regs->ADDR = config->base_addr + addr;
		regs->CTLR |= FLASH_CTLR_STRT;
...
#define FLASH_CH32V_INIT(idx)                                                                      \
	static const struct flash_ch32v_config flash_ch32v_##idx##_config = {                      \
		...
		.base_addr = DT_REG_ADDR(DT_INST(idx, soc_nv_flash)),                              \
