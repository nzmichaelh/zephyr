...
/* The SAM0 series uses different and conflicting names for the
 * erasable units and programmable units:
 *
...
{
	if (offset < 0) {
		SYS_LOG_DBG("%x: before start of flash", offset);
		return -EINVAL;
	}
	if ((offset + len) > CONFIG_FLASH_SIZE * 1024) {
		SYS_LOG_DBG("%x: ends past the end of flash", offset);
		return -EINVAL;
	}
...
static int flash_sam0_check_status(off_t offset)
{
	...
	if (status.bit.PROGE) {
		SYS_LOG_ERR("programming error at %x", offset);
		return -EIO;
	} else if (status.bit.LOCKE) {
		SYS_LOG_ERR("lock error at %x", offset);
		return -EROFS;
	} else if (status.bit.NVME) {
		SYS_LOG_ERR("NVM error at %x", offset);
		return -EIO;
	}
				 ...
				 const void *data)
{
	...
	if (err != 0) {
		SYS_LOG_ERR("%x: write error", offset);
		return err;
	}

	if (memcmp(data, FLASH_MEM(offset), FLASH_PAGE_SIZE) != 0) {
		SYS_LOG_ERR("%x: verify error", offset);
		return -EIO;
	}
...
static int flash_sam0_write(struct device *dev, off_t offset, const void *data,
			    size_t len)
{
	struct flash_sam0_data *ctx = dev->driver_data;
	...
	for (addr = offset; addr < offset + len; addr++) {
		...
		if (base != ctx->offset) {
			SYS_LOG_DBG("%x: fill on new row", base);
			    ...
			    size_t len)
{
	...
	if ((offset % FLASH_PAGE_SIZE) != 0) {
		SYS_LOG_ERR("%x is not on a write block boundrary", offset);
		return -EINVAL;
	}

	if ((len % FLASH_PAGE_SIZE) != 0) {
		SYS_LOG_ERR("%x is not a integer number of write blocks", len);
		return -EINVAL;
	}
...
static int flash_sam0_erase(struct device *dev, off_t offset, size_t size)
{
	...
	SYS_LOG_DBG("%x: size %u", offset, size);
	...
	if ((offset % ROW_SIZE) != 0) {
		SYS_LOG_ERR("%x is not on a page boundrary", offset);
		return -EINVAL;
	}

	if ((size % ROW_SIZE) != 0) {
		SYS_LOG_ERR("%x is not a integer number of pages", size);
		return -EINVAL;
	}
...
DEVICE_AND_API_INIT(flash_sam0, CONFIG_SOC_FLASH_SAM0_DEV_NAME, flash_sam0_init,
