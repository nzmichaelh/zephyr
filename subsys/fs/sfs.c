...
ssize_t fs_write(fs_file_t *zfp, const void *ptr, size_t size)
{
	...
	block = sfs_find(zfp->fp.name, hdr);
	if (block >= 0) {
		err = sfs_erase(block);
		if (err != 0) {
	...
	hdr->magic = SFS_MAGIC;
	hdr->size = size;
	sfs_copy_name(hdr->name, zfp->fp.name);
 ...
 * and supports fs_seek to rewind after the first read has been done.
 */
ssize_t fs_read(fs_file_t *zfp, void *ptr, size_t size)
{
	...
	if (zfp->fp.block < 0) {
		...
		err = sfs_find(zfp->fp.name, &hdr);

		if (err < 0) {
			return err;
		}
		zfp->fp.block = err;
		zfp->fp.size = hdr.size;
	}

	remain = zfp->fp.size - zfp->fp.offset;
	read = min(remain, size);

	if (read > 0) {
		err = flash_read(data.dev,
				 sfs_to_offset(zfp->fp.block) +
...
int fs_readdir(fs_dir_t *zdp, struct fs_dirent *entry)
{
	...
	for (; zdp->dp.block < SFS_NUM_BLOCKS &&
	       sfs_read_header(zdp->dp.block, &hdr) != 0;
	     zdp->dp.block++) {
	}

	if (!sfs_valid_block(zdp->dp.block)) {
		entry->name[0] = '\0';
		return 0;
	}
...
int fs_open(fs_file_t *zfp, const char *filename)
{
	 ...
	 * read or write.
	 */
	sfs_copy_name(zfp->fp.name, filename);
...
int fs_seek(fs_file_t *zfp, off_t offset, int whence)
{
	...
	case FS_SEEK_SET:
		...
		break;
	case FS_SEEK_CUR:
		target = zfp->fp.offset + whence;
		break;
	case FS_SEEK_END:
		target = zfp->fp.size - whence;
		break;
	default:
		return -EINVAL;
	}
	if (target < 0 || target > zfp->fp.size) {
		return -EINVAL;
	}
	zfp->fp.offset = target;
